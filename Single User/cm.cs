...
namespace Clinical_Management
{
    class cm
    {
        public static string cs = "Data Source=(local)\\cm_express;Initial Catalog=cm;uid=cm_usr;pwd=123456";
            //"Data Source=(local);Initial Catalog=cm;uid=cm_usr;pwd=123456";
            //"Data Source=(local);Initial Catalog=cm;uid=cm_usr;pwd=123456";
        //"Data Source=169.254.114.0;Initial Catalog=cm;uid=cm_usr;pwd=123456";
        // "Data Source=(local);Initial Catalog=cm;Integrated Security=True";
        // "Data Source=192.168.0.1;Initial Catalog=cm;uid=cm_usr;pwd=123456";
        // "Data Source=192.168.0.2;Initial Catalog=cm;uid=cm_usr;pwd=123456";

        //************************************************
        public static Int64 date2day(String mydate)
        {
            String date1 = mydate;
            Int32 i = date1.IndexOf('/');
            String sy = date1.Substring(0, i);
            Int32 y = Int32.Parse(sy);
            date1 = date1.Substring(i + 1, date1.Length - i - 1);
            i = date1.IndexOf('/');
            String sm = date1.Substring(0, i);
            Int32 m = Int32.Parse(sm);
            date1 = date1.Substring(i + 1, date1.Length - i - 1);
            String sd = date1;
            Int32 d = Int32.Parse(sd);
            Int64 result = (y * 365) + (m * 30) + d;
            return result;
        }
        //************************************************
        public static string MyEncoding(String str)
        {
            Encoding myacsii = Encoding.ASCII;

            Byte[] bb = myacsii.GetBytes(str);
            string s = "";
        ...
        public static string MyDecoding(String str)
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                string temp = s.Substring(0,j);
                  bb[i] = Byte.Parse((Byte.Parse(temp) - 25).ToString());
                try
                {
        ...
        private static ArrayList ClLefts = new ArrayList();
        ...
        public static void Print_DataGridView(DataGridView dgv1)
        {
            ...
            PageSetupDialog p1 = new PageSetupDialog();
            p1.Document = printDoc;
            p1.MinMargins.Bottom = 200;
            p1.MinMargins.Left = 200;
            p1.MinMargins.Right = 200;
            p1.MinMargins.Top = 200;
            p1.ShowDialog();
                    ...
                    System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewColumn GridCol in dgv.Columns)
                {
                    ...
                    if (!cm.SelectedColumns.Contains(GridCol.HeaderText)) continue;
                    ...
                    System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            //MessageBox.Show(ColumnLefts.Count.ToString());
            ...
            try
            {
                ...
                if (PageNo == 1)
                {
                    ...
                    //////// My Code //////////
                    int mytmp;
                    for (int k = 0; k < ColumnLefts.Count / 2; k++)
                    {
                        mytmp = (int)ColumnLefts[k];
                        ColumnLefts[k] = ColumnLefts[ColumnLefts.Count - k - 1];
                        ColumnLefts[ColumnLefts.Count - k - 1] = mytmp;
                    }

                    for (int k = 0; k < ColumnWidths.Count / 2; k++)
                    {
                        mytmp = (int)ColumnWidths[k];
                        ColumnWidths[k] = ColumnWidths[ColumnWidths.Count - k - 1];
                        ColumnWidths[ColumnWidths.Count - k - 1] = mytmp;
                    }
                    ...
                    while (RowPos <= dgv.Rows.Count - 1)
                    {
                        ...
                        else
                        {
                            if (NewPage)
                            {
                                ...
                                {
                                    if (!GridCol.Visible) continue;
                                    if (!cm.SelectedColumns.Contains(GridCol.HeaderText))
                                        continue;
                    ...
                    int RowsPerPage)
        {
            ...
            string ss = Math.Ceiling((double)(cnt / RowsPerPage)).ToString();
            if (ss == "0")
                ss = "1";
